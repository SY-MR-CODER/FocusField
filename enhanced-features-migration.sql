...
-- Update user stats table with new fields
ALTER TABLE simple_user_stats 
ADD COLUMN IF NOT EXISTS total_focus_sessions INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS average_focus_duration DECIMAL(5,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS productivity_score INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS habits_completed INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS themes_unlocked INTEGER DEFAULT 0;
...
INSERT INTO simple_themes (name, colors, unlocked, unlock_condition) VALUES
...
SELECT 
  ...
  COALESCE(AVG(p.health_level), 0) as average_plant_health,
  COUNT(DISTINCT a.id) as total_achievements,
  up.theme_id,
  up.daily_goal_tasks,
  up.daily_goal_focus_minutes
FROM simple_users u
LEFT JOIN simple_user_stats us ON u.id = us.user_id
LEFT JOIN simple_streaks s ON u.id = s.user_id
LEFT JOIN simple_plants p ON u.id = p.user_id
LEFT JOIN simple_achievements a ON u.id = a.user_id
LEFT JOIN simple_user_preferences up ON u.id = up.user_id
GROUP BY u.id, u.username, u.display_name, us.total_tasks_completed, 
         us.total_focus_minutes, us.total_focus_sessions, us.current_streak, 
         us.best_streak, us.level, us.experience_points, us.productivity_score,
         up.theme_id, up.daily_goal_tasks, up.daily_goal_focus_minutes;
...
INSERT INTO simple_habits (user_id, name, description, frequency, target_count, icon) 
SELECT id, 'Drink Water', 'Stay hydrated throughout the day', 'daily', 8, '💧' FROM simple_users LIMIT 1;

INSERT INTO simple_habits (user_id, name, description, frequency, target_count, icon) 
SELECT id, 'Exercise', 'Get some physical activity', 'daily', 1, '🏃' FROM simple_users LIMIT 1;

INSERT INTO simple_habits (user_id, name, description, frequency, target_count, icon) 
SELECT id, 'Read', 'Read for personal development', 'daily', 1, '📚' FROM simple_users LIMIT 1;
*/
